...
BASE="https://fapi.binance.com";WS="wss://fstream.binance.com/market";WEBHOOK=os.getenv("FEISHU_WEBHOOK","")
API_GAP=.45;FEISHU_GAP=2;RANK_REFRESH=3600;MAX429=3
caches,btc_cache,ws_data={},{},{};ws_symbols=[];ws_conn=None;ws_thread=rank_thread=None
...
NAMES={"15m":"15分钟","1H":"1小时","4H":"4小时","1D":"1天"};TYPES={"TREND":"趋势","BREAKOUT":"突破"};DIRS={"LONG":"做多","SHORT":"做空"}
...
def get(path,p=None,retry=3):
    ...
    for i in range(retry):
        ...
        try:
            ...
            if r.status_code in (403,418):breaker=True;return
...
def coins(n=150):
    ...
    if not a:return []
    s=[x["symbol"] for x in a["symbols"] if x.get("quoteAsset")=="USDT" and x.get("status")=="TRADING" and x.get("contractType")=="PERPETUAL"]
...
def ws_open(w):
    ...
    a=streams(s);ws_send(w,"SUBSCRIBE",a,1);print(f"[WS] {len(s)}币种/{len(a)}流")
...
def ws_loop():
    ...
    while not ws_stop:
        try:
            w=websocket.WebSocketApp(WS,on_open=ws_open,on_message=ws_msg,on_close=ws_close)
            w.run_forever(ping_interval=None,ping_timeout=None)
        except:pass
        if ws_stop:break
        time.sleep(d+random.uniform(.5,2));d=min(d*2,60)
...
def update_symbols(ns):
    ...
    print(f"[排名] 新增{len(add)} 移除{len(rem)} 当前{len(ns)}")
...
def signal(df,tf,sym):
    ...
    def score(d):
        s=30 if vr>=2 else 15 if vr>=1.5 else 0;s+=25 if body/rng>=.7 else 12 if body/rng>=.55 else 0;s+=20 if a>aa*1.05 else 0;s+=min(max(bs if d=="LONG" else -bs,0),15);s+=10 if (d=="LONG" and p>e20) or (d=="SHORT" and p<e20) else 0;return s
    c=[]
    ...
    if not c:return
    d,t,s,sl,tp,rr=max(c,key=lambda x:x[2]);return {"sym":sym,"tf":tf,"dir":d,"type":t,"score":s,"entry":p,"sl":sl,"tp":tp,"rr":rr}
...
def evaluate(s):
    ...
    for _,r in future.iloc[:bars].iterrows():
        if s["dir"]=="LONG":sl,tp=r.l<=s["sl"],r.h>=s["tp"]
        else:sl,tp=r.h>=s["sl"],r.l<=s["tp"]
...
def main():
    ...
    cutoff=(datetime.now(timezone.utc)-timedelta(hours=48)).timestamp();sent={k:v for k,v in sent.items() if v>cutoff}
    syms=coins(150)
    if not syms:print("[错误] 未获取到币种列表");return
    print(f"[启动] {len(syms)}个币种");start_ws(syms);time.sleep(2)
    for tf in TF:
        if breaker:print("[中断] 触发限流保护");break
    ...
    save("sent_cache.json",sent);save("signals_record.json",records);print("本轮扫描结束")
