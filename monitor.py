...
ws_symbols=[]
ws_conn=None
ws_thread=rank_thread=None
ws_lock=threading.Lock()
breaker=ws_stop=False
con429=0
...
def ok_t(tf):
    ...
    if tf=="1D":return n.hour==0 and 30<=n.minute<45
    if tf=="4H":return n.hour%4==0 and 15<=n.minute<30
    if tf=="1H":return n.minute<15
    return True
...
def streams(s):
    return [
        f"{x.lower()}@kline_{v['bar']}"
        for x in s for v in TF.values()
    ]

def ws_send(ws,method,a,start):
...
def ws_open(ws):
    ...
    with ws_lock:
        ws_conn=ws
        s=list(ws_symbols)
    ws_send(ws,"SUBSCRIBE",streams(s),1)
    print(f"[WS] {len(s)}币种/{len(streams(s))}流")
    ...
    while not ws_stop:
        try:
            w=websocket.WebSocketApp(
                WS,on_open=ws_open,
                on_message=ws_msg,on_close=ws_close
            )
            w.run_forever(ping_interval=None,ping_timeout=None)
        except:pass
        if ws_stop:break
        time.sleep(d+random.uniform(.5,2))
        d=min(d*2,60)
...
def update_symbols(ns):
    ...
    for s in rem:
        for v in TF.values():
            ws_data.pop(f"{s}_{v['bar']}",None)
            caches.pop(f"{s}_{v['bar']}",None)
...
def start_ws(s):
    ...
    if not rank_thread or not rank_thread.is_alive():
        rank_thread=threading.Thread(target=rank_loop,daemon=True)
        rank_thread.start()
...
def candles(sym,bar,limit=250):
    ...
    if a and len(a)>=limit:
        df=pd.DataFrame(a[-limit:],columns=["ts","o","h","l","c","v"])
        caches[k]=(now,df)
        return df
...
def atr(df,n=14):
    p=df.c.shift()
    return pd.concat([
        df.h-df.l,(df.h-p).abs(),(df.l-p).abs()
...
def signal(df,tf,sym):
    ...
    p=df.c.iloc[-1]
    e20,e60,e120,e200=[ema(df.c,n).iloc[-1] for n in (20,60,120,200)]
    a=atr(df).iloc[-1]
    if not a:return
